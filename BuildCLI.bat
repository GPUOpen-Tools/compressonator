@echo off
::-----------------------------------------------------------------
:: Build file for Compressonator CLI
:: Copyright - AMD 2016-2018
::
::
:: Syntax for this batch file
::    cd <compressonator-root-folder>
::    set VULKAN_SDK=<vulkan-sdk-root>
::    .\BuildCLI.bat
::
:: Revision History
::-----------------------------------------------------------------
...
set msbuildStatus=%ERRORLEVEL%
IF %msbuildStatus% GTR 0 goto Error
::
Goto Done
:Error
::
...
echo ***** MSBuild Exit Error = %msbuildStatus%                     >> %OUTPUT_LOG%
echo ------------------------------------------------------------   >> %OUTPUT_LOG%
::
cd %BatchDir%
echo "BUILD FAILED"
